...
export default function DoctorAuth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      // Check if email is authorized for doctor signup
      if (email.toLowerCase() !== DOCTOR_EMAIL) {
        toast({
          variant: "destructive",
          title: "Unauthorized",
          description: "This email is not authorized for doctor registration.",
        });
        setIsLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen login-bg flex items-center justify-center p-4 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 right-10 w-72 h-72 login-accent-bg rounded-full blur-3xl animate-float" />
        <div className="absolute bottom-20 left-10 w-96 h-96 login-accent-bg rounded-full blur-3xl animate-float" style={{ animationDelay: '1.5s' }} />
      </div>
